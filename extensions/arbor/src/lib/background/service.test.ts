...
import { BACKUP_ALARM, type BackupRecord, type BackupRepository } from "../backups";
...
import type { AlarmsPort, AlarmSchedule, SettingsStore } from "./ports";
...
function fakeAlarms() {
  const alarms = new Map<string, AlarmSchedule>();
  ...
  return { port, alarms, fire: (name: string) => listeners.forEach((l) => l(name)) };
}
...
async function harness(options: { pro?: boolean; settings?: Settings } = {}) {
  ...
  let clock = 1_000;
  const pro = { value: options.pro ?? false };
  ...
  await service.ready;
  return { service, store, fb, w, alarms, backups, settings, pro };
}
...
describe("ArborBackground startup", () => {
  ...
  it("arms the backup alarm only for Pro users with backups enabled", async () => {
    const settings: Settings = {
      ...DEFAULT_SETTINGS,
      backups: { ...DEFAULT_SETTINGS.backups, enabled: true, intervalMinutes: 30 },
    };
    const { alarms } = await harness({ pro: true, settings });
    expect(alarms.alarms.get(BACKUP_ALARM)).toEqual({ periodInMinutes: 30, delayInMinutes: 30 });
  });

  it("re-arms or stops backups when settings or the licence change", async () => {
    const { service, alarms, settings, pro } = await harness({ pro: true });
    expect(alarms.alarms.has(BACKUP_ALARM)).toBe(false);
    settings.change({
      ...DEFAULT_SETTINGS,
      backups: { ...DEFAULT_SETTINGS.backups, enabled: true, intervalMinutes: 15 },
    });
    await tick();
    expect(alarms.alarms.get(BACKUP_ALARM)).toEqual({ periodInMinutes: 15, delayInMinutes: 15 });
    pro.value = false;
...
describe("ArborBackground alarms", () => {
  it("runs a scheduled backup and trims to the retention", async () => {
    const settings: Settings = {
      ...DEFAULT_SETTINGS,
      backups: { ...DEFAULT_SETTINGS.backups, enabled: true, retention: 2 },
    };
    const { service, backups } = await harness({ pro: true, settings });
    for (let i = 0; i < 3; i++) {
  ...
  it("clears the backup alarm when Pro lapsed", async () => {
    const settings: Settings = {
      ...DEFAULT_SETTINGS,
      backups: { ...DEFAULT_SETTINGS.backups, enabled: true },
    };
    const { service, alarms, backups, pro } = await harness({ pro: true, settings });
    pro.value = false;
    service.handleAlarm(BACKUP_ALARM);
    await tick();
    expect(alarms.alarms.has(BACKUP_ALARM)).toBe(false);
