...
describe("primary action (Enter / double-click)", () => {
  it("focuses a live tab and a live window", () => {
    expect(primaryActionFor(node("tab", { liveTabId: 5, liveWindowId: 1 }))).toBe("focus");
    expect(primaryActionFor(node("window", { liveWindowId: 1 }))).toBe("focus");
  });

  it("restores (reopens in place) a saved tab, a saved window, a group and a note", () => {
    expect(primaryActionFor(node("tab", { url: "https://a.test/" }))).toBe("restore");
    expect(primaryActionFor(node("window"))).toBe("restore");
    expect(primaryActionFor(node("group"))).toBe("restore");
