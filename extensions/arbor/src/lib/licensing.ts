/**
 * Arbor's Lemon Squeezy licensing wiring. Owns the build-time configuration (read from WXT env
 * vars), the single `LicenseClient` per JS context, and small helpers for the UI. The client itself
 * lives in `@browserforge/licensing`; this file only decides *how* Arbor uses it.
 *
